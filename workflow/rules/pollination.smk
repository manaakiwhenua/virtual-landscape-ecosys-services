rule pollinator_requirement:
    ...
    log:
        "logs/pollination/{landscape}/{topography}.{landclass}.LandscapePollinatorRequirement.log"
    shell:
...
rule pollinator_quality:
    input:
        f"{config['basepath']}"+"/virtual-landscapes/{landscape}/landclass_{topography}_{landclass}.tif"
    output:
        temp("results/pollination/{landscape}/{topography}.{landclass}.LandscapePollinatorQuality.tif")
    params:
        datatype="UInt16",
        landscape="/virtual-landscapes/{landscape}/landclass_{topography}_{landclass}.tif",
    container:
        "docker://osgeo/gdal:ubuntu-small-latest"
    log:
        "logs/pollination/{landscape}/{topography}.{landclass}.LandscapePollinatorQuality.log"
    shell:
...
rule grassdata_landclass:
    ...
    params:
        landscape="/virtual-landscapes/{landscape}/landclass_{topography}_{landclass}.tif",
    log:
        "logs/grass/data/region/{landscape}/{topography}/{landclass}.grass.log"
    shell:
...
rule pollinator_quality_grow:
    input:
        ...
        q="results/pollination/{landscape}/{topography}.{landclass}.LandscapePollinatorQuality.tif"
    output:
        d=temp("results/pollination/{landscape}/{topography}.{landclass}.LandscapePollinatorQuality_distance.tif"),
        v=temp("results/pollination/{landscape}/{topography}.{landclass}.LandscapePollinatorQuality_value.tif")
    container:
        "docker://neteler/grassgis7:latest"
    params:
        quality_layer="{landscape}_{topography}_{landclass}_LandscapePollinatorQuality_grow",
        metric="euclidean",
        grass="grass -f /results/grass/data/region/{landscape}/{topography}/{landclass}/PERMANENT --text --exec"
    log:
        "logs/pollination/{landscape}/{topography}.{landclass}.LandscapePollinatorQuality_value.log"
...
rule pollinator_quality_expand:
    input:
        q="results/pollination/{landscape}/{topography}.{landclass}.LandscapePollinatorQuality.tif",
        d="results/pollination/{landscape}/{topography}.{landclass}.LandscapePollinatorQuality_distance.tif",
        v="results/pollination/{landscape}/{topography}.{landclass}.LandscapePollinatorQuality_value.tif"
    output:
        temp("results/pollination/{landscape}/{topography}.{landclass}.LandscapePollinatorQuality_expand.tif")
    container:
        "docker://osgeo/gdal:ubuntu-small-latest"
    params:
        datatype="UInt16",
    ...
    log:
        "logs/pollination/{landscape}/{topography}.{landclass}.LandscapePollinatorQuality_expand.log"
...
rule pollinator_overlap:
    input:
        q="results/pollination/{landscape}/{topography}.{landclass}.LandscapePollinatorQuality_expand.tif",
        r="results/pollination/{landscape}/{topography}.{landclass}.LandscapePollinatorRequirement.tif"
    output:
    ...
    params:
        datatype="UInt16"
    log:
        "logs/pollination/{landscape}/{topography}.{landclass}.LandscapePollinatorOverlap.log"
    shell:
